...
if (isset($_POST['addproduct'])) {
  ...
  //     $selQ = "SELECT `product_id`, `product_name`, `product_details`, `product_designer`, `category`, `acro`, `price`, `size`, `img` FROM aces_inventory WHERE `product_id` = $add LIMIT 1;";
  ...
  //     $status = "NEW"; $date = date('Y-m-d'); $time =  date('h:i:s');
  //     for($i=0; $i < $qty; $i++){
  //       $query = "INSERT INTO `aces_inventory`(`product_id`, `product_name`, `product_details`, `product_designer`, `category`, `acro`, `price`, `size`, `img`, `qty`, `date_added`, `time_added`) VALUES ( '$PRODUCTID', '$PNAME','$PDES','$PRODESIGN','$CATEGORY','$PCOURSE','$PRICE','$PSIZE','$PIMG',1,'$date','$time');";
